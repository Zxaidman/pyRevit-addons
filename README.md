...
| **Bridge Check**        | Ask the **AnonGee Bridge** whether it is running, from inside Revit. pyRevit serves HTTP routes from inside the Revit process, and an Excel button calls them; when Excel gets no answer the five possible reasons look identical from Excel. This calls the same two URLs from the machine Revit is on. `ping` takes no Revit argument and answers even mid-command; `status` runs as an External Event and answers only once Revit is idle — so ping answering while status does not is a finding, not a failure. Read-only. |
...
| **RC Automation**       | Drives reinforcement from an Excel schedule. Checks the workbook against **BS 8666:2020**, plans what would be built — creating the footings a schedule places, or reinforcing ones already modelled — and says why anything is skipped, then builds it. Level names are matched rather than demanded: `Ground` finds `00 Ground Lvl.`. Nothing is written until Create, and the whole run is **one undo step**. A footing that already has reinforcement is left alone; a pad that is not the size it was scheduled is reported rather than reinforced wrongly. Reads `.xlsx`/`.xlsm`, a folder of delimited sheets, or one file with `#SHEET` rows. Bars are bent to their shape code and kept inside the cover, each footing face gets its own named cover type, and every handle is constrained to the cover face it is nearest — which is what makes a varying set follow a tapered pad's sloping edge. The project's own identity fields (`ID`, `ID_LIC`, `ID_V`, `ITEM`, `LEVEL_V`, and `Host Category`/`Host Mark` on bars) are filled in; missing shared parameters are created only if you ask. |
