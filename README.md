...
| **RC Automation**       | Drives reinforcement from an Excel schedule. Checks the workbook against **BS 8666:2020**, plans what would be built — creating the footings a schedule places, or reinforcing ones already modelled — and says why anything is skipped, then builds it. Level names are matched rather than demanded: `Ground` finds `00 Ground Lvl.`. Nothing is written until Create, and the whole run is **one undo step**. A footing that already has reinforcement is left alone; a pad that is not the size it was scheduled is reported rather than reinforced wrongly. Reads `.xlsx`/`.xlsm`, a folder of delimited sheets, or one file with `#SHEET` rows. |
