# -*- coding: utf-8 -*-
"""The planar-graph engine every slab outline is built on.

A slab boundary is a FACE: take the lines that bound it -- drawn slab edges, the
edges of placed beams, the footprint rings of placed columns -- weld their
endpoints, split them where they cross, and walk the bounded faces of the graph
that results. This module is that machinery, and nothing in it knows what a
slab is.

The parts that matter, and why each exists:

    _cluster_nodes        welds endpoints that mean the same junction, PREFERRING
                          beam-edge points so a boundary lands exactly on the
                          placed member rather than drifting onto a column ring
    _junction_weights     tells a free tip (weld it) from a T the geometry pins
                          (leave it), which is what keeps a real 50 mm step from
                          collapsing into a slope
    _split_at_crossings   cuts edges at T and X junctions so a face can close
    _walk_faces           the traversal itself
    _snap_ring_to_carriers  the exactness pass: every vertex re-derived onto the
                          member line it came from, so the slab edge and the beam
                          edge are the SAME line, not two lines 2 mm apart
    _square_off_chamfers  a corner welded across two carriers reads as a chamfer;
                          this puts the corner back

The tolerances live here too, with `apply_tolerances`, because they are what the
welding and healing steps mean by "close": the dialog's Tolerances tab writes
them once per run and every reader sees the same values.

Revit-free and unit-tested against synthetic graphs.
"""
