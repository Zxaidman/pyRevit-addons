#! python3
# -*- coding: utf-8 -*-
# pyRevit script - Copy Rebar Visibility (CPython 3)
...
# Note: 'from Autodesk.Revit.UI import *' is safely removed since you 
# are handling UI elements through __revit__.ActiveUIDocument
    ...
    <Grid Margin="15,10">
        ...
        <Border Grid.Row="0" Background="#2C3E50" Padding="12,8" Margin="0,0,0,10" CornerRadius="4">
            <Grid>
                ...
                <TextBlock Grid.Column="0" Text="Copy Rebar Visibility"
        ...
        <StackPanel Grid.Row="1" Margin="0,0,0,5">
            <Label Content="Source View (Copy From):" Style="{StaticResource SectionLabel}" Margin="0,0,0,5" />
            <ComboBox x:Name="source_combo" Style="{StaticResource ControlCombo}" Height="28" />
        </StackPanel>

        <!-- TARGET VIEWS LABEL -->
        <Label Grid.Row="2" Content="Target Views (Apply To):" Style="{StaticResource SectionLabel}" Margin="0,5,0,5" />

        <!-- TARGET VIEWS LISTBOX -->
        <ListBox Grid.Row="3" x:Name="views_list" Background="White"
        ...
        <Grid Grid.Row="4" Margin="0,0,0,0">
            ...
            <StackPanel Grid.Column="0" Orientation="Horizontal">
                <Button x:Name="btn_all" Content="All" Style="{StaticResource SecondaryBtn}"
                        Width="60" ToolTip="Select all target views" />
                <Button x:Name="btn_none" Content="None" Style="{StaticResource SecondaryBtn}"
                        Width="60" Margin="5,0,0,0" ToolTip="Clear all target views" />
            </StackPanel>
            <Button Grid.Column="2" x:Name="btn_apply" Content="Apply &#x2192;"
                    Style="{StaticResource ActionButton}" Width="140"
                    ToolTip="Copy rebar visibility from source to selected target views" />
        </Grid>

        <!-- STATUS BAR -->
        <Border Grid.Row="5" Background="#E8E8E8" Padding="12,8" Margin="0,15,0,10" CornerRadius="4">
            <Grid>
                <Grid.ColumnDefinitions>
                    <ColumnDefinition Width="Auto" />
                    <ColumnDefinition Width="*" />
                ...
                <TextBlock Grid.Column="2" Text="Copy Rebar Visibility v2.0 | pyZaid"
                           Style="{StaticResource StatusText}" VerticalAlignment="Center" />
            </Grid>
        </Border>

        <!-- BUTTON BAR -->
        <Border Grid.Row="6" Background="{StaticResource LightBgBrush}">
...
def run():
    ...
    source_combo    = window.FindName("source_combo")
    ...
    # 3. Populate UI
    # ------------------------------------------------------------------
    live_count.Text = "{} rebars".format(len(all_rebars))

    # Build parallel data list: [(view, display_name), ...]
    ...
    # Default source to active view
    active_id = get_id_value(doc.ActiveView.Id)
    for i, (view, name) in enumerate(views_data):
        if get_id_value(view.Id) == active_id:
            source_combo.SelectedIndex = i
            break
    ...
    def get_selected_target_views():
        ...
        for i in range(views_list.Items.Count):
            ...
            chk = item.Content
            if chk.IsChecked:
                selected.append(views_data[i][0])
        return selected
    ...
    def on_apply(sender, args):
        src_idx = source_combo.SelectedIndex
        if src_idx < 0:
            set_status("Please select a source view.", is_error=True)
            return

        source_view = views_data[src_idx][0]
        target_views = get_selected_target_views()

        # Prevent applying to itself
        source_id = get_id_value(source_view.Id)
        target_views = [v for v in target_views if get_id_value(v.Id) != source_id]

        if not target_views:
            set_status("Select at least one valid target view (not the source).", is_error=True)
            return

        set_status("Processing {} rebars...".format(len(all_rebars)))
        force_ui_update()
        ...
        try:
            for t_view in target_views:
                ...
                # Batch hide/unhide for performance
                elements_to_hide = List[ElementId]()
                elements_to_unhide = List[ElementId]()

                for rebar in all_rebars:

                    # 1. Copy Hidden Status
                    try:
                        is_hidden_in_source = rebar.IsHidden(source_view)
                        if is_hidden_in_source:
                            if not rebar.IsHidden(t_view) and rebar.CanBeHidden(t_view):
                                elements_to_hide.Add(rebar.Id)
                        else:
                            if rebar.IsHidden(t_view) and rebar.CanBeHidden(t_view):
                                elements_to_unhide.Add(rebar.Id)
                    except:
                        pass

                    # 2. Copy Unobscured Status
                    try:
                        if hasattr(rebar, "IsUnobscuredInView"):
                            is_unobs = rebar.IsUnobscuredInView(source_view)
                            if rebar.IsUnobscuredInView(t_view) != is_unobs:
                                rebar.SetUnobscuredInView(t_view, is_unobs)
                    except:
                        pass

                    # 3. Copy Solid Status (Only copy if Source is 3D and Target is 3D)
                    try:
                        if source_is_3d and t_is_3d and hasattr(rebar, "IsSolidInView"):
                            is_solid = rebar.IsSolidInView(source_view)
                            if rebar.IsSolidInView(t_view) != is_solid:
                                rebar.SetSolidInView(t_view, is_solid)
                    except:
                        pass

                # Apply Batch Hide/Unhide
                if elements_to_hide.Count > 0:
                    try:
                        t_view.HideElements(elements_to_hide)
                    except:
                        pass

                if elements_to_unhide.Count > 0:
                    try:
                        t_view.UnhideElements(elements_to_unhide)
                    except:
                        pass

            t.Commit()
            set_status("Visibility copied from '{}' to {} view(s)".format(
