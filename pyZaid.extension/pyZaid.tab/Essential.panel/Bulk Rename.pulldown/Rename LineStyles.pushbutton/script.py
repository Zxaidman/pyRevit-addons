...
class FindReplaceDialog(Window):
    ...
    def __init__(self):
        self.Title = "Find & Replace Line Styles"
        self.Width = 350
        self.Height = 180
...
def bulk_rename_linestyles_combined_ui():
    ...
    try:
        ...
        forms.alert("Could not access model Line Styles: {}".format(e), exitscript=True)
        return

    renamed_count = 0
    
    with revit.Transaction("Bulk Rename Line Styles (Console Output)"):
        for subcategory in lines_category.SubCategories:
            ...
            if is_match:
                category_element = doc.GetElement(subcategory.Id)
                if category_element is None:
                    continue

                try:
                    category_element.Name = new_name
                    renamed_count += 1
                    # Log successful rename out to pyRevit CLI window
                    print("SUCCESS: Renamed Line Style '{}' -> '{}'".format(current_name, new_name))
                except Exception as err:
                    print("SKIPPED: System-locked style '{}' could not be modified. Reason: {}".format(current_name, err))

    # Print summary block directly to the log window instead of launching an intrusive alert window
    print("\n" + "="*50)
    print("LINE STYLE PROCESSING SUMMARY")
    print("="*50)
    print("Total line styles successfully updated: {}".format(renamed_count))
    print("="*50 + "\n")

if __name__ == "__main__":
    bulk_rename_linestyles_combined_ui()
