...
class FindReplaceDialog(Window):
    ...
    def __init__(self):
        self.Title = "Find & Replace Fill Patterns"
        self.Width = 350
        self.Height = 180
...
def bulk_rename_fillpatterns_combined_ui():
    ...
    # Fetch all FillPatternElements in the current document
    try:
        fill_patterns = DB.FilteredElementCollector(doc)\
                          .OfClass(DB.FillPatternElement)\
                          .ToElements()
    except Exception as e:
        forms.alert("Could not access model Fill Patterns: {}".format(e), exitscript=True)
        return

    renamed_count = 0
    
    with revit.Transaction("Bulk Rename Fill Patterns (Console Output)"):
        for pattern_element in fill_patterns:
            ...
            if is_match:
                try:
                    ...
                    print("SUCCESS: Renamed Fill Pattern '{}' -> '{}'".format(current_name, new_name))
    ...
    print("="*50)
    print("Total fill patterns successfully updated: {}".format(renamed_count))
    print("="*50 + "\n")

if __name__ == "__main__":
    bulk_rename_fillpatterns_combined_ui()
