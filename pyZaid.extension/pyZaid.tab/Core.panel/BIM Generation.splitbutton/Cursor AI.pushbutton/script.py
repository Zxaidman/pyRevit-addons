...
def get_floor_type(base_type, thickness_mm):
    ...
    # FORMAT: "100 THK. RCC SLAB"
    nm = "{} THK. RCC SLAB".format(int(round(thickness_mm)))
...
class InpToBimDialog(Window):
    ...
    def _run(self, sender, args):
        ...
        try:
            ...
            self.update_progress(10, "Initializing Generation...")
            ...
            for idx, (name, base_z_m, top_z_m, pts) in enumerate(col_data):
                if total_cols > 0 and idx % 5 == 0: 
                    self.update_progress(10 + int((float(idx) / total_cols) * 20), "Generating Columns {}/{}...".format(idx, total_cols))
                ...
                # Standardized pattern "C4 @4.0m" (using top_z_m)
            ...
            for idx, (name, top_z_m, pts) in enumerate(beam_data):
                if total_beams > 0 and idx % 5 == 0: 
                    self.update_progress(30 + int((float(idx) / total_beams) * 30), "Generating Beams {}/{}...".format(idx, total_beams))
                pts_m = [XYZ(p[0], p[1], p[2]) for p in pts]
            ...
            for idx, (name, top_z_m, base_z_m, pts) in enumerate(slab_data):
                if total_slabs > 0 and idx % 2 == 0: 
                    self.update_progress(60 + int((float(idx) / total_slabs) * 20), "Generating Slabs {}/{}...".format(idx, total_slabs))
            ...
            self.update_progress(80, "Joining Geometry: Executing Overlap Calculations...")
            ...
            # Restrict joins only to elements that legitimately penetrate to avoid warning
            # "Highlighted elements are joined but do not intersect."
            def overlaps(bb1, bb2, tol=-0.05):
            ...
            total_cols_j = len(all_cols)
            for i, col in enumerate(all_cols):
                if total_cols_j > 0 and i % 10 == 0: 
                    self.update_progress(80 + int((float(i) / total_cols_j) * 10), "Joining Columns to Slabs...")
                bbc = bb(col)
                # Removed Column -> Beam manual join logic to prevent Revit interaction errors.
                for floor in all_floors:
                    if overlaps(bbc, bb(floor)): try_join_ordered(col, floor)

            total_beams_j = len(all_beams)
            for i, beam in enumerate(all_beams):
                if total_beams_j > 0 and i % 10 == 0: 
                    self.update_progress(90 + int((float(i) / total_beams_j) * 5), "Joining Beams to Slabs...")
                bbb = bb(beam)
                for floor in all_floors:
            ...
            # Completion sequence - Keep popup alive, finalize regeneration
            doc.Regenerate()  # Make model visually stable
